...
export default function Dashboard() {
  ...
  const todayFiber = todayLog?.fiberGrams ?? 0;
...
function WeeklyProteinWidget({
  ...
  return (
    <div className="bg-surface rounded-xl border border-border p-4 flex flex-col gap-3">
      ...
      {done ? (
        <p className="text-3xl font-extrabold text-success">Goal Reached! 🎉</p>
      ) : (
        <div className="flex items-baseline gap-1">
          <span className="text-3xl font-extrabold text-accent">{remaining}g</span>
          <span className="text-secondary text-sm font-semibold">LEFT</span>
        </div>
      )}
