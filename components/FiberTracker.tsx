...
interface FiberTrackerProps {
  counts: FiberCounts;
  onChange: (counts: FiberCounts) => void;
  goal: number;
}

export default function FiberTracker({ counts, onChange, goal }: FiberTrackerProps) {
  const { sources: allSources } = useFiberSources();
  const sources = allSources.filter((s) => s.active);

  const total = sources.reduce((sum, source) => {
    return sum + (counts[source.key] ?? 0) * source.fiber;
  }, 0);
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="bg-surface rounded-xl p-3 border border-border">
        ...
        <ProgressBar value={total} max={goal} color="var(--color-success)" />
      </div>

      {sources.map((source) => {
        const qty = counts[source.key] ?? 0;
        const grams = qty * source.fiber;
        return (
          <div
            key={source.key}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-secondary">
                {grams > 0 ? `${grams}g fiber` : `${source.fiber}g per ${source.unit}`}
              </p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                disabled={qty === 0}
                className="w-9 h-9 rounded-lg bg-border text-app-text font-bold disabled:opacity-30 hover:bg-success/20 transition-colors text-lg leading-none flex items-center justify-center"
              ...
              <button
                type="button"
                onClick={() => adjust(source.key, 1)}
                className="w-9 h-9 rounded-lg bg-success text-bg font-bold hover:bg-success/80 transition-colors text-lg leading-none flex items-center justify-center"
